...
function setPallet(input) {
    if (input.includes(" ")) {//if there are spaces
        let cs = input.split(" ");
        for (let i = 0; i < cs.length; i++) {//loop through each color
            if (cs[i].charAt(0) != '#') {//if the first character is not a #
        ...
        setup()
    } else {//if no spaces just push the color
        if (input.charAt(0) != '#') {
            input = '#' + input;
        }
        colors.push(input);
